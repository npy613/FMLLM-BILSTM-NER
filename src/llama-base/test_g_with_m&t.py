...
def evaluate_model(model, val_dataloader, device, measure_time=False, warmup_samples=100):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(val_dataloader):
            ...
            if processed_samples >= warmup_samples:
                per_sample_time = batch_time / len(predictions)
                inference_times.extend([per_sample_time] * len(predictions)) 
    ...
    if measure_time:
        if warmup_samples > 0:
            print(f"Warm-up样本数: {min(warmup_samples, len(all_true_labels))}")
        if len(inference_times) > 0:
            avg_inference_time = np.mean(inference_times)
            p50 = np.percentile(inference_times, 50)
            p95 = np.percentile(inference_times, 95)
            p99 = np.percentile(inference_times, 99)
            print(f"平均每个样本推理时间 (排除warm-up): {avg_inference_time*1000:.2f} ms")
            print(f"P50推理时延: {p50*1000:.2f} ms")
            print(f"P95推理时延: {p95*1000:.2f} ms")
            print(f"P99推理时延: {p99*1000:.2f} ms")
            print(f"实际测量样本数: {len(inference_times)}")
            return avg_val_loss, all_predictions, all_true_labels, avg_inference_time
        else:
            print("警告: 所有样本都用于warm-up，未记录推理时间")
...
def load_model(model, load_path, device):
    ...
    if 'ner_classifier_state_dict' in checkpoint:
        model.ner_classifier.load_state_dict(checkpoint['ner_classifier_state_dict'])
        print("NER分类器加载成功")
...
if __name__ == "__main__":
    ...
    max_length = 128
    hidden_dim = 128
    batch_size = 1  # 8 ： 4bit模型大约需要6GB显存
    ...
    result = evaluate_model(fresh_model, val_dataloader, device, measure_time=True)
    if len(result) == 4:
        final_val_loss, final_predictions, final_true_labels, avg_inference_time = result
    else:
        final_val_loss, final_predictions, final_true_labels = result
    ...
    if torch.cuda.is_available():
        ...
        print(f"推理过程额外显存: {peak_memory - model_memory:.2f} MB")

    
    if avg_inference_time is not None:
        print(f"平均每个样本推理时延: {avg_inference_time*1000:.2f} ms")
